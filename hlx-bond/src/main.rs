...
impl GgufTokenizer {
    fn from_gguf(content: &gguf_file::Content) -> Result<Self> {
        ...
        if let Some(gguf_file::Value::Array(merges)) =
        ...
        let im_start_id = vocab_map
            .get("<|im_start|>")
            .copied()
            .unwrap_or(u32::MAX);
        let im_end_id = vocab_map
            .get("<|im_end|>")
            .copied()
            .unwrap_or(u32::MAX);
...
impl CorpusContext {
    fn open(path: &str) -> Result<Self> {
        let conn = Connection::open(path)
            .with_context(|| format!("Failed to open corpus at {}", path))?;
        Ok(Self { conn })
    }
    ...
    fn build_system_prompt(&self, max_rules: usize, max_memory: usize) -> Result<String> {
        ...
        let mut stmt = self.conn.prepare(
            "SELECT role, content FROM memory ORDER BY created_at DESC LIMIT ?1",
        )?;
...
fn run_repl(
    ...
    loop {
        ...
        for h in 0..args.h_cycles {
            ...
            let prompt_tokens =
                tokenizer.encode_chat(&system, &history, &user_input);
...
fn main() -> Result<()> {
    ...
    println!("\n[bond] Session ended. Steps taken: {}", symbiote_state.step_count);
    Ok(())
}
