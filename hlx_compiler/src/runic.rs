...
impl Parser for RunicParser {
    fn parse(&self, source: &str) -> Result<Program> {
        // For now, convert runic to HLXL and parse that
        // In a full implementation, this would be a native runic parser
        let hlxl_source = transliterate_to_hlxl(source)?;
        crate::HlxlParser::new().parse(&hlxl_source)
    }
    ...
    fn emit_stmt(&mut self, stmt: &Statement, out: &mut String) -> Result<()> {
        ...
        match stmt {
            ...
            Statement::For { variable, iterator, body } => {
                out.push_str(glyphs::FOR);
                out.push_str(" (");
                out.push_str(variable);
                out.push_str(" in ");
                self.emit_expr(&iterator.node, out)?;
                out.push_str(") {\n");
