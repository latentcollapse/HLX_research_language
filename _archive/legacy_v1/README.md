**Developer's note: HLX is an experimental runtime and substrate, not a drop-in replacement for existing ML frameworks. This is early and rough, and was rapidly prototyped using Claude. The idea behind HLX is that English is full of entropy. LLMs "think" in a structured way, and so the idea behind this project was that by providing LLMs with a native substrate that mimics the way they structure data, it could reduce hallucinations and enable more precise data transfer. This won't eliminate hallucinations in LLMs, but the theory is that by removing the noise, it could allow them to hit much higher benchmarks. The compiler and runtimes to make it executable came after the bootstrap capsule containing the original language, and was simply the result of me experimenting with Vulkan. Full disclaimer: On my hardware (NVIDIA 8gb 5060) it ran 1.3x slower, but produced better convergence by 6.7%, interestingly enough. I've been able to reproduce the results on my hardware, but this has not been tested on AMD or Intel hardware. It'll likely shatter if forked to AMD/Intel because I've not been doign this very long and still haven't nailed repro across operating systems and hardware, but I'm working on it.**

**TL:DR this weird format I made edged CUDA in loss parity by almost 7%. Why? Dunno yet, but I've got theories**
...
**COMPILER**
It's driven by a Rust implementation of the full transformer training logic (src/bin/train_transformer_full.rs) that's compiled into a binary, using the custom Vulkan training logic, shader loading, and optimizer.
...
Installs dependencies, compiles shaders, builds binary, and runs benchmark.

```bash
curl -sSL https://raw.githubusercontent.com/latentcollapse/hlx-compiler/main/install_bootstrap.sh | bash
```

### 🪟 Windows / AMD
We provide a cross-platform Python runner. Ensure [Rust](https://rustup.rs/) and [Vulkan SDK](https://vulkan.lunarg.com/) are installed.
...
### 🐳 Docker
Build the compiler in a clean container (GPU access requires NVIDIA Container Toolkit).

```bash
docker build -t hlx-compiler .
docker run --gpus all hlx-compiler
```

### 🐧 Arch Linux
Use the `PKGBUILD` included in this repo:
...
1. [LANGUAGE_SPECIFICATION.md](docs/LANGUAGE_SPECIFICATION.md) - Overview (15 min read)
2. [HLXL_SPECIFICATION.md](docs/HLXL_SPECIFICATION.md) - Learn ASCII syntax (30 min)
...
|----------|-------|------|----------|
| [LANGUAGE_SPECIFICATION.md](docs/LANGUAGE_SPECIFICATION.md) | Language family overview + quick reference | 8KB | Everyone |
| [HLXL_SPECIFICATION.md](docs/HLXL_SPECIFICATION.md) | ASCII language - full syntax & semantics | 18KB | Developers |
| [HLX_SPECIFICATION.md](docs/HLX_SPECIFICATION.md) | Runic language - glyph form | 12KB | Advanced users |
| [WIRE_FORMATS.md](docs/WIRE_FORMATS.md) | LC-B, LC-R, LC-T serialization | 20KB | Implementers |
| [CONTRACT_SYSTEM.md](docs/CONTRACT_SYSTEM.md) | Schema validation & type safety | 15KB | Architects |
| [TYPE_SYSTEM.md](docs/TYPE_SYSTEM.md) | Type semantics & constraints | 14KB | Language designers |
...
```
[LANGUAGE_SPECIFICATION.md](docs/LANGUAGE_SPECIFICATION.md) (overview)
  ↓
[HLXL_SPECIFICATION.md](docs/HLXL_SPECIFICATION.md) (learn to write)
  ↓
[examples/](examples/) (run code)
```
...
Path 1 (above)
  ↓
[TYPE_SYSTEM.md](docs/TYPE_SYSTEM.md) (understand types)
  ↓
[CONTRACT_SYSTEM.md](docs/CONTRACT_SYSTEM.md) (learn contracts)
  ↓
[WIRE_FORMATS.md](docs/WIRE_FORMATS.md) (understand serialization)
```
...
Path 2 (above)
  ↓
[HLX_SPECIFICATION.md](docs/HLX_SPECIFICATION.md) (glyph form)
  ↓
