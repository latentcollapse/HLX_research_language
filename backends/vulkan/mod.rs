...
use crate::backend::{Backend, DType, TensorHandle, TensorMeta};
use crate::config::RuntimeConfig;
use crate::tuning::{detect_vendor, BackendTuning, GenericTuning};
use hlx_core::{HLXError, Result, Value};
...
fn verify_shader(shader: &ShaderInfo) -> Result<()> {
    let computed = compute_sha256(shader.bytes);
    if computed != shader.expected_hash {
        return Err(HLXError::BackendError {
            message: format!(
                "Shader attestation failed: {} - expected {}, got {}",
                shader.name, shader.expected_hash, computed
            ),
        });
    }
    Ok(())
...
const ALL_SHADERS: &[ShaderInfo] = &[
    ...
    ShaderInfo {
        name: "activation",
        bytes: SHADER_ACTIVATION,
        expected_hash: "",
    },
    ShaderInfo {
        name: "softmax",
        bytes: SHADER_SOFTMAX,
        expected_hash: "",
    },
    ShaderInfo {
        name: "layernorm",
        bytes: SHADER_LAYERNORM,
        expected_hash: "",
    },
    ShaderInfo {
        name: "cross_entropy",
        bytes: SHADER_CROSS_ENTROPY,
        expected_hash: "",
    },
    ShaderInfo {
        name: "elementwise",
        bytes: SHADER_ELEMENTWISE,
        expected_hash: "",
    },
    ShaderInfo {
        name: "reduction",
        bytes: SHADER_REDUCTION,
        expected_hash: "",
    },
    ShaderInfo {
        name: "conv2d",
    ...
    ShaderInfo {
        name: "pooling",
        bytes: SHADER_POOLING,
        expected_hash: "",
    },
    ShaderInfo {
        name: "batchnorm",
        bytes: SHADER_BATCHNORM,
        expected_hash: "",
    },
    ShaderInfo {
        name: "dropout",
        bytes: SHADER_DROPOUT,
        expected_hash: "",
    },
    ShaderInfo {
        name: "transpose",
        bytes: SHADER_TRANSPOSE,
        expected_hash: "",
    },
    ShaderInfo {
        name: "gaussian_blur",
        bytes: SHADER_GAUSSIAN_BLUR,
        expected_hash: "",
    },
    ShaderInfo {
        name: "sobel",
    ...
    ShaderInfo {
        name: "grayscale",
        bytes: SHADER_GRAYSCALE,
        expected_hash: "",
    },
    ShaderInfo {
        name: "threshold",
        bytes: SHADER_THRESHOLD,
        expected_hash: "",
    },
    ShaderInfo {
        name: "brightness",
        bytes: SHADER_BRIGHTNESS,
        expected_hash: "",
    },
    ShaderInfo {
        name: "contrast",
        bytes: SHADER_CONTRAST,
        expected_hash: "",
    },
    ShaderInfo {
        name: "invert_colors",
        bytes: SHADER_INVERT_COLORS,
        expected_hash: "",
    },
    ShaderInfo {
        name: "sharpen",
        bytes: SHADER_SHARPEN,
        expected_hash: "",
    },
    ShaderInfo {
        name: "basic_vert",
        bytes: SHADER_BASIC_VERT,
        expected_hash: "",
    },
    ShaderInfo {
        name: "basic_frag",
        bytes: SHADER_BASIC_FRAG,
        expected_hash: "",
    },
    ShaderInfo {
        name: "pbr_frag",
        bytes: SHADER_PBR_FRAG,
        expected_hash: "",
    },
];

pub fn verify_all_shaders(strict: bool) -> Result<usize> {
    let mut verified = 0;
    for shader in ALL_SHADERS {
        if shader.expected_hash.is_empty() {
            if strict {
                return Err(HLXError::BackendError {
                    message: format!("Shader {} has no attestation hash", shader.name),
                });
            }
        } else {
...
pub fn compute_all_shader_hashes() -> Vec<(&'static str, String)> {
    ALL_SHADERS
        .iter()
        .map(|s| (s.name, compute_sha256(s.bytes)))
        .collect()
}

impl VulkanBackend {
    pub fn new(_config: &RuntimeConfig) -> Result<Self> {
        verify_all_shaders(false)?;
        ...
        unsafe {
            ...
            let instance =
                entry
                    ...
                    .map_err(|e| HLXError::BackendError {
            ...
            let pdevices =
                instance
                    ...
                    .map_err(|e| HLXError::BackendError {
                        message: format!("No physical devices: {}", e),
                    })?;
            ...
            let (pdevice, queue_family_index) = pdevices
                ...
                .ok_or(HLXError::BackendError {
                    message: "No compute-capable GPU got".to_string(),
                })?;
            ...
            let device = instance
                ...
                .map_err(|e| HLXError::BackendError {
                    message: format!("Device creation failed: {}", e),
                })?;
            ...
            .map_err(|e| HLXError::BackendError {
                message: format!("Allocator init failed: {}", e),
            })?;
            ...
            let command_pool = device
                ...
                .map_err(|e| HLXError::BackendError {
                    message: format!("Command pool creation failed: {}", e),
                })?;
            ...
            let transfer_command_buffer = device
                ...
                .map_err(|e| HLXError::BackendError {
                message: format!("Command buffer alloc failed: {}", e),
            })?[0];

            let fence_create_info = vk::FenceCreateInfo::builder();
            let transfer_fence = device.create_fence(&fence_create_info, None).map_err(|e| {
                HLXError::BackendError {
                    message: format!("Fence creation failed: {}", e),
                }
            })?;
            ...
            let descriptor_set_layout = device
                ...
                .map_err(|e| HLXError::BackendError {
                    message: format!("DS layout failed: {}", e),
                })?;
            ...
            let pipeline_layout = device
                ...
                .map_err(|e| HLXError::BackendError {
                    message: format!("Pipeline layout failed: {}", e),
                })?;
    ...
    fn get_or_create_pipeline(&mut self, name: &str, spv: &[u8]) -> Result<vk::Pipeline> {
        ...
        unsafe {
            ...
            for i in 0..code_len {
                let bytes = [spv[i * 4], spv[i * 4 + 1], spv[i * 4 + 2], spv[i * 4 + 3]];
            ...
            let shader_module = self
                ...
                .map_err(|e| HLXError::BackendError {
                    message: format!("Shader module failed: {}", e),
                })?;
    ...
    fn activation(&mut self, input: TensorHandle, out: TensorHandle, mode: u32) -> Result<()> {
        ...
        unsafe {
            ...
            let descriptor_set = self
                ...
                .map_err(|e| HLXError::BackendError {
                    message: format!("Set alloc failed: {}", e),
                })?[0];
            ...
            let info_in = vk::DescriptorBufferInfo {
                buffer: buf_in,
                offset: 0,
                range: vk::WHOLE_SIZE,
            };
            let info_out = vk::DescriptorBufferInfo {
                buffer: buf_out,
                offset: 0,
                range: vk::WHOLE_SIZE,
            };
            ...
            self.device
                .cmd_dispatch(self.transfer_command_buffer, group_count, 1, 1);

            self.device
                .end_command_buffer(self.transfer_command_buffer)
                .map_err(|e| HLXError::BackendError {
                    message: format!("End failed: {}", e),
                })?;

            let submit_info = vk::SubmitInfo::builder()
                .command_buffers(std::slice::from_ref(&self.transfer_command_buffer));
            self.device
                .queue_submit(self.queue, &[submit_info.build()], self.transfer_fence)
                .map_err(|e| HLXError::BackendError {
...
impl Backend for VulkanBackend {
    ...
    fn alloc_tensor(&mut self, shape: &[usize], dtype: DType) -> Result<TensorHandle> {
        ...
        println!(
            "[Vulkan] Allocating Tensor: {:?} ({:?}) -> {} bytes",
        ...
        unsafe {
            ...
            let buffer = self.device.create_buffer(&buffer_info, None).map_err(|e| {
                HLXError::BackendError {
                    message: format!("Buffer creation failed: {}", e),
                }
            })?;
    ...
    fn write_tensor(&mut self, handle: TensorHandle, data: &[u8]) -> Result<()> {
        ...
        unsafe {
            ...
            let staging_buffer = self
                ...
                .map_err(|e| HLXError::BackendError {
                    message: format!("Staging buffer failed: {}", e),
                })?;
            ...
            let allocation = allocator
                ...
                .map_err(|e| HLXError::BackendError {
                    message: format!("Staging alloc failed: {}", e),
                })?;

            // 2. Map and Copy
            let ptr = allocation.mapped_ptr().ok_or(HLXError::BackendError {
                message: "Failed to map staging buffer".to_string(),
            })?;
            std::ptr::copy_nonoverlapping(data.as_ptr(), ptr.as_ptr() as *mut u8, data.len());

            self.device
                .bind_buffer_memory(staging_buffer, allocation.memory(), allocation.offset())
                .map_err(|e| HLXError::BackendError {
                    message: format!("Staging bind failed: {}", e),
                })?;
            ...
            self.device
                .begin_command_buffer(self.transfer_command_buffer, &begin_info)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Begin command failed: {}", e),
                })?;
            ...
            self.device
                .end_command_buffer(self.transfer_command_buffer)
                .map_err(|e| HLXError::BackendError {
                    message: format!("End command failed: {}", e),
                })?;
            ...
            self.device
                .wait_for_fences(&[self.transfer_fence], true, u64::MAX)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Wait fence failed: {}", e),
                })?;
            self.device
                .reset_fences(&[self.transfer_fence])
                .map_err(|e| HLXError::BackendError {
                    message: format!("Reset fence failed: {}", e),
                })?;
            ...
            allocator
                .free(allocation)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Free staging failed: {}", e),
                })?;
        }
        Ok(())
    }
    ...
    fn scalar_sqrt(&mut self, a: &Value) -> Result<Value> {
        match a {
            ...
            _ => Err(hlx_core::HLXError::TypeError {
    ...
    fn pointwise_add(&mut self, a: TensorHandle, b: TensorHandle, out: TensorHandle) -> Result<()> {
        ...
        unsafe {
            ...
            let info_a = vk::DescriptorBufferInfo {
                buffer: buf_a,
                offset: 0,
                range: vk::WHOLE_SIZE,
            };
            let info_b = vk::DescriptorBufferInfo {
                buffer: buf_b,
                offset: 0,
                range: vk::WHOLE_SIZE,
            };
    ...
    fn matmul(&mut self, a: TensorHandle, b: TensorHandle, out: TensorHandle) -> Result<()> {
        ...
        unsafe {
            ...
            let group_y = (m + 15) / 16;
            self.device
                .cmd_dispatch(self.transfer_command_buffer, group_x, group_y, 1);
    ...
    fn layer_norm(
        ...
        unsafe {
            ...
            let mean_buffer = self
                .device
                .create_buffer(&mean_buffer_info, None)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Mean buffer failed: {}", e),
                })?;
            ...
            let var_buffer = self
                .device
                .create_buffer(&var_buffer_info, None)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Var buffer failed: {}", e),
                })?;
            ...
            let mean_alloc = allocator
                ...
                .map_err(|e| HLXError::BackendError {
                    message: format!("Mean alloc failed: {}", e),
                })?;

            let var_alloc = allocator
                .allocate(&AllocationCreateDesc {
                ...
                .map_err(|e| HLXError::BackendError {
                    message: format!("Var alloc failed: {}", e),
                })?;
            drop(allocator);

            self.device
                .bind_buffer_memory(mean_buffer, mean_alloc.memory(), mean_alloc.offset())
                .map_err(|e| HLXError::BackendError {
                    message: format!("Mean bind failed: {}", e),
                })?;
            self.device
                .bind_buffer_memory(var_buffer, var_alloc.memory(), var_alloc.offset())
                .map_err(|e| HLXError::BackendError {
                    message: format!("Var bind failed: {}", e),
                })?;
            ...
            let push_pass0 = LayerNormPushConstants {
                batch_size,
                hidden_size,
                epsilon: eps as f32,
                pass: 0,
            ...
            self.device
                .cmd_dispatch(self.transfer_command_buffer, 1, batch_size, 1);
            ...
            // Pass 1: Normalize
            let push_pass1 = LayerNormPushConstants {
                batch_size,
                hidden_size,
                epsilon: eps as f32,
                pass: 1,
            ...
            // Cleanup
            self.device
                .destroy_descriptor_set_layout(layernorm_layout, None);
            self.device.destroy_descriptor_pool(descriptor_pool, None);
            ...
            allocator
                .free(mean_alloc)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Free mean failed: {}", e),
                })?;
            allocator
                .free(var_alloc)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Free var failed: {}", e),
                })?;
        }
    ...
    fn softmax(&mut self, input: TensorHandle, out: TensorHandle, _dim: i32) -> Result<()> {
        ...
        unsafe {
            ...
            let max_buffer = self
                .device
                .create_buffer(&max_buffer_info, None)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Max buffer failed: {}", e),
                })?;
            ...
            let sum_buffer = self
                .device
                .create_buffer(&sum_buffer_info, None)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Sum buffer failed: {}", e),
                })?;
            ...
            let max_alloc = allocator
                ...
                .map_err(|e| HLXError::BackendError {
                    message: format!("Max alloc failed: {}", e),
                })?;

            let sum_alloc = allocator
                .allocate(&AllocationCreateDesc {
                    name: "Softmax Sum",
                ...
                .map_err(|e| HLXError::BackendError {
                    message: format!("Sum alloc failed: {}", e),
                })?;
            drop(allocator);

            self.device
                .bind_buffer_memory(max_buffer, max_alloc.memory(), max_alloc.offset())
                .map_err(|e| HLXError::BackendError {
                    message: format!("Max bind failed: {}", e),
                })?;
            self.device
                .bind_buffer_memory(sum_buffer, sum_alloc.memory(), sum_alloc.offset())
                .map_err(|e| HLXError::BackendError {
                    message: format!("Sum bind failed: {}", e),
                })?;
            ...
            let push_pass0 = SoftmaxPushConstants {
                batch_size,
                seq_len,
                pass: 0,
                _padding: 0,
            ...
            // Pass 1: Normalize
            let push_pass1 = SoftmaxPushConstants {
                batch_size,
                seq_len,
                pass: 1,
                _padding: 0,
            ...
            // Cleanup
            self.device
                .destroy_descriptor_set_layout(softmax_layout, None);
            self.device.destroy_descriptor_pool(descriptor_pool, None);
            ...
            allocator
                .free(max_alloc)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Free max failed: {}", e),
                })?;
            allocator
                .free(sum_alloc)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Free sum failed: {}", e),
                })?;
        }
    ...
    fn reduce_sum(
        ...
        let (input_size, reduce_size, output_size) = if let Some(d) = dim {
            let d = if d < 0 {
                (meta.shape.len() as i32 + d) as usize
            } else {
                d as usize
            };
        ...
        unsafe {
            ...
            self.device
                .cmd_dispatch(self.transfer_command_buffer, 1, output_size as u32, 1);
    ...
    fn dispatch_compute(
        ...
        unsafe {
            ...
            println!(
                "[Vulkan] Creating DS Layout with {} bindings",
                bindings.len()
            );
            std::io::stdout().flush().ok();
            ...
            println!(
                "[Vulkan] Creating Pipeline Layout (PC size: {})",
                push_constants.len()
            );
            std::io::stdout().flush().ok();
            ...
            // 3. Create Pipeline
            println!(
                "[Vulkan] Creating Shader Module ({} bytes)",
                shader_bytes.len()
            );
            std::io::stdout().flush().ok();
    ...
    fn gaussian_blur(
        ...
        Err(HLXError::BackendError {
            message: "gaussian_blur shader dispatch not yet implemented for Vulkan backend"
                .to_string(),
        })
    }
    ...
    fn sobel_edges(
        ...
        Err(HLXError::BackendError {
            message: "sobel_edges shader dispatch not yet implemented for Vulkan backend"
                .to_string(),
        })
    }
        ...
        unsafe {
            ...
            let descriptor_set =
                self.device
                    .allocate_descriptor_sets(&alloc_info)
                    .map_err(|e| HLXError::BackendError {
                        message: format!("Descriptor set allocation failed: {}", e),
                    })?[0];
            ...
            let push_constants = GrayscalePushConstants {
                width,
                height,
                channels,
            };
            ...
            self.device
                .begin_command_buffer(self.transfer_command_buffer, &begin_info)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Command buffer begin failed: {}", e),
                })?;
            ...
            self.device
                .end_command_buffer(self.transfer_command_buffer)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Command buffer end failed: {}", e),
                })?;
            ...
            self.device
                .wait_for_fences(&[self.transfer_fence], true, u64::MAX)
                .map_err(|e| HLXError::BackendError {
                    message: format!("Fence wait failed: {}", e),
                })?;
            self.device
                .reset_fences(&[self.transfer_fence])
                .map_err(|e| HLXError::BackendError {
                    message: format!("Fence reset failed: {}", e),
                })?;
    ...
    fn threshold(&mut self, input: TensorHandle, out: TensorHandle, value: &Value) -> Result<()> {
        ...
        self.dispatch_image_shader_with_param(
            input,
            out,
            "threshold",
            SHADER_THRESHOLD,
    ...
    fn sync(&mut self) -> Result<()> {
        unsafe {
            self.device
                .device_wait_idle()
                .map_err(|e| HLXError::BackendError {
                    message: format!("Wait idle failed: {}", e),
                })
        }
    }
}
...
impl VulkanBackend {
    ...
    fn dispatch_image_shader_simple(
        ...
        unsafe {
            ...
            let push_constants = SimplePushConstants {
                width,
                height,
                channels,
            };
    ...
    fn dispatch_image_shader_with_param(
        ...
        unsafe {
            ...
            let push_constants = ParamPushConstants {
                width,
                height,
                channels,
                param,
